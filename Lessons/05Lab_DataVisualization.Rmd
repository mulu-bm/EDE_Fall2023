...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    ...
    #expression() is used here since there are nested formulas.
    #If there are no nested formulas, we can just use paste().
    )
...
library(dplyr)
SurfaceTemperaturePlot_Task1 <- PeterPaul.chem.nutrients %>% filter(depth==0) %>% 
ggplot(mapping=aes(x=daynum,y=temperature_C,color=year4)) + geom_point()+facet_wrap(vars(lakename))+labs(y="Surface Temperature")

SurfaceTemperaturePlot_Task1
...
SurfaceTemperaturePlot_Task2 <- PeterPaul.chem.nutrients %>% 
ggplot(mapping=aes(x=sampledate,y=temperature_C,color=depth)) + geom_point(size=0.5)+labs(y="Surface Temperature")+scale_color_viridis(option = "magma", direction = -1)+scale_x_date(date_breaks = "5 years",date_labels = "%Y")

SurfaceTemperaturePlot_Task2
...
AQIPlot_Task3 <- EPAair %>% 
ggplot(mapping=aes(x=PM2.5,y=Ozone,color=mean_Lat)) + geom_point(alpha=0.5)+scale_color_viridis(name="Latitude",option="viridis",direction = -1)+geom_smooth(method="lm")
