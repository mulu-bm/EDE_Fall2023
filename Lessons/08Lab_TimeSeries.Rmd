...
plot.BR_complete.Rainfall <- BR_complete %>% 
  ggplot(aes(x=Date,y=Rainfall)) +
  geom_line(color="blue") +
  labs(x="Date",
       y="Rainfall",
       title="Rainfall by time")+
  geom_smooth(method=lm,color="black")
print(plot.BR_complete.Rainfall)

plot.BR_complete.Temp <- BR_complete %>% 
...
rainfall_data_ts <- ts(BR_complete$Rainfall,
                   start=c(first_year,first_month),
                   frequency=12) 

temperature_data_ts <- ts(BR_complete$Temperature_C,
                   start=c(first_year,first_month),
                   frequency=12)
...
rainfall_data_decomp <- stl(rainfall_data_ts,s.window = "periodic")
temperature_data_decomp <- stl(temperature_data_ts,s.window = "periodic")

plot(rainfall_data_decomp)
plot(temperature_data_decomp)
...
rainfallDataDecomp_Components <- as.data.frame(rainfall_data_decomp$time.series[,1:3])

temperatureDataDecomp_Components <- as.data.frame(temperature_data_decomp$time.series[,1:3])
