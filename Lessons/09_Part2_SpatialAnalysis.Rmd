...
epa_pm25_sites_sf <- read_csv(here('EDE_Fall2023/Data/Raw/EPAair_PM25_NC2018_raw.csv')) %>% 
...
#
counties_sf<- st_read(here('EDE_Fall2023/Data/Spatial/cb_2018_us_county_20m.shp')) %>% 
...
huc8_sf<- st_read(here('EDE_Fall2023/Data/Spatial/NCHUC8.shp'))
...
neuse_sf<- st_read(here('EDE_Fall2023/Data/Spatial/NCHUC8.shp')) %>% 
  filter(SUBBASIN == "Upper Neuse") #Filter 


#Create a map
mapView(huc8_sf, col.regions = 'orange', layer.name='HUC 8') +
  mapView(neuse_sf, col.regions = 'purple')
...
neuse_intersect_3 <- counties_sf_utm %>% 
  st_filter(x = ., y = neuse_sf, sparse = FALSE)

neuse_intersect_4 <- counties_sf_utm %>% 
  filter(st_is_within_distance( x=., y = neuse_sf, dist=100000, sparse = FALSE))
test <- st_is_within_distance( x=counties_sf_utm, y = neuse_sf, dist=100000, sparse = FALSE)
