...
#Get the data, which starts on line 31
gage_data <- read.table(theURL,skip = 30,header=TRUE,sep='\t',stringsAsFactors = T) 
...
the_price <- the_website %>% html_nodes('td:nth-child(1)') %>% html_text()
the_capacity <- the_website %>% html_nodes('td:nth-child(1)') %>% html_text()
net_generation <- the_website %>% html_nodes('td:nth-child(1)') %>% html_text()
total_retail <- the_website %>% html_nodes('td:nth-child(1)') %>% html_text()
